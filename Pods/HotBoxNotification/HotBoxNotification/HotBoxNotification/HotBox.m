...
- (void)showMessage:(NSString *)text ofType:(NSString *)type duration:(NSTimeInterval)timeout
{
    [self showMessage:[[NSAttributedString alloc] initWithString:text] ofType:type withDelegate:nil duration:timeout];
}
...
- (void)presentViewControllerWithText:(NSAttributedString *)text ofType:(NSString *)type withDelegate:(id<HotBoxDelegate>)delegate buttonTitle:(NSString *)buttonTitle
{
    if(![self.viewControllers lastObject]
       ...
       || (![[[self.viewControllers lastObject] buttonTitle] isEqualToString:buttonTitle] && buttonTitle.length)) {
        ...
        vc.messageLabel.font = [UIFont fontWithName:@"Raleway-Bold" size:14];
        vc.messageLabel.textColor = [UIColor whiteColor];
